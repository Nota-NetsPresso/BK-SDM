...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        help="Used in loading text encoder and image decoder; Path to pretrained model or model identifier from huggingface.co/models."
        )    
    parser.add_argument(
        "--unet_path",
        type=str,
        default="nota-ai/bk-sdm-small",
        help="Used in loading unet; Path to pretrained model or model identifier from huggingface.co/models."
        )    
    parser.add_argument(
        "--revision",
        type=str,
...
def main(args):
    ...
    # Add custom logging
    val_img_dir = Path(args.output_dir, 'val_img')
    log_path_loss = Path(args.output_dir, 'loss.csv')
    ...
    if accelerator.is_main_process:
        if args.push_to_hub: # todo: delete
            ...
            with open(log_path_loss, 'w') as logfile:
                logwriter = csv.writer(logfile, delimiter=',')
                if args.with_prior_preservation:
                    logwriter.writerow(['global_step','epoch','step','total','instance','prior'])
                else:
                    logwriter.writerow(['global_step','epoch','step','loss'])
    ...
    unet = UNet2DConditionModel.from_pretrained(
        args.unet_path, subfolder='unet', revision=args.revision
    )
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                with accelerator.accumulate(unet):                        
                    ...
                    if args.with_prior_preservation:
                        ...
                        inst_loss = F.mse_loss(model_pred.float(), target.float(), reduction="mean")

                        # Compute prior loss
                        prior_loss = F.mse_loss(model_pred_prior.float(), target_prior.float(), reduction="mean")                        

                        # Add the prior loss to the instance loss.
                        loss = inst_loss + args.prior_loss_weight * prior_loss

                        with open(log_path_loss, 'a') as logfile:
                            logwriter = csv.writer(logfile, delimiter=',')
                            logwriter.writerow([global_step, epoch, step, loss.item(), inst_loss.item(), prior_loss.item()])
                        ...
                        with open(log_path_loss, 'a') as logfile:
                            logwriter = csv.writer(logfile, delimiter=',')
                            logwriter.writerow([global_step, epoch, step, loss.item()])
                ...
                if (
                    ...
                    for kk in range(args.num_validation_images):
                        ...
                        images.append(image)
                        tmp_name = Path(val_img_dir, f"gstep{global_step}_epoch{epoch}_step{step}_{kk}.png")
                        image.save(tmp_name)
                        print(tmp_name)
    ...
    if accelerator.is_main_process:
        if args.use_lora:
            ...
            accelerator.save(state_dict, os.path.join(args.output_dir, "lora.pt"))
